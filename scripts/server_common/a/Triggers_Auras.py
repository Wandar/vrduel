...
class TrigBoard:
	...
	def connect(self):
		game, camp = self.entity.Game, self.entity.camp
		for sig in self.signals:
			try: game.trigsBoard[camp][sig].append(self)
			except: game.trigsBoard[camp][sig] = [self]
			
	def disconnect(self):
		game, camp = self.entity.Game, self.entity.camp
		for sig in self.signals:
			try: game.trigsBoard[camp][sig].remove(self)
			except: pass
			
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		return True
		
	def trig(self, signal, camp, subject, target, number, comment, choice=0):
		if self.canTrig(signal, camp, subject, target, number, comment):
			if self.entity.Game.GUI:
				self.entity.Game.GUI.trigBlink(self.entity, blockwhilePlaying=self.blockwhilePlaying)
			self.effect(signal, camp, subject, target, number, comment)
	...
	#一般只有需要额外定义camp的回合开始和结束扳机需要有自己的selfCopy函数
...
class TrigHand:
	...
	def connect(self):
		for sig in self.signals:
			try: self.entity.Game.trigsHand[self.entity.camp][sig].append(self)
			except: self.entity.Game.trigsHand[self.entity.camp][sig] = [self]
			
	def disconnect(self):
		for sig in self.signals:
			try: self.entity.Game.trigsHand[self.entity.camp][sig].remove(self)
			except: pass
...
class TrigDeck:
	...
	def connect(self):
		for sig in self.signals:
			try: self.entity.Game.trigsDeck[self.entity.camp][sig].append(self)
			except: self.entity.Game.trigsDeck[self.entity.camp][sig] = [self]
			
	def disconnect(self):
		for sig in self.signals:
			try: self.entity.Game.trigsDeck[self.entity.camp][sig].remove(self)
			except: pass
...
class Deathrattle_Monster(TrigBoard):
	def __init__(self, entity):
		self.blank_init(entity)
		
	def blank_init(self, entity):
		self.entity = entity
		self.signals = ["MonsterDies", "TrigDeathrattle"]
		self.inherent = True
		
	def trig(self, signal, camp, subject, target, number, comment, choice=0):
		monster, game = self.entity, self.entity.Game
		if game.status[monster.camp]["Deathrattle x2"] > 0:
			if self.canTrig(signal, camp, subject, target, number, comment):
				if game.GUI: game.GUI.deathrattleAni(monster, color="grey40")
				self.effect(signal, camp, subject, target, number, comment)
		if self.canTrig(signal, camp, subject, target, number, comment):
			if game: game.GUI.deathrattleAni(monster, color="grey40")
			self.effect(signal, camp, subject, target, number, comment)
		#随从通过死亡触发的亡语扳机需要在亡语触发之后注销。同样的，如果随从在亡语触发之后不在随从列表中了，如将随从洗回牌库，则同样要注销亡语
		#但是如果随从在由其他效果在场上触发的扳机，则这个亡语不会注销
		if signal[0] == 'M' or monster not in game.monsters[monster.camp]:
			self.disconnect()
			
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		return self.entity.Game.status[self.entity.camp]["Deathrattle X"] < 1 and target == self.entity
...
class Deathrattle_Weapon(TrigBoard):
	...
	def trig(self, signal, camp, subject, target, number, comment, choice=0):
		...
		if game.status[weapon.camp]["Weapon Deathrattle x2"] > 0:
			if self.canTrig(signal, camp, subject, target, number, comment):
...
class SecretTrigger(TrigBoard):
	...
	def connect(self):
		secret = self.entity
		game, camp = secret.Game, secret.camp
		for sig in self.signals:
			try: game.trigsBoard[camp][sig].append(self)
			except: game.trigsBoard[camp][sig] = [self]
	...
	def disconnect(self):
		...
		#	#game.trigAuras[camp].remove(self)
		#else:
	...
	def trig(self, signal, camp, subject, target, number, comment, choice=0):
		...
		try: game.Secrets.secrets[secret.camp].remove(secret)
		except: pass
		if game.status[secret.camp]["Secrets x2"] > 0:
			if self.canTrig(signal, camp, subject, target, number, comment):
		...
		game.Counters.numSecretsTriggeredThisGame[secret.camp] += 1
...
#这个扳机的目标：当随从在回合结束时有多个同类扳机，只会触发第一个，这个可以通过回合camp和自身camp是否符合来决定
class Trig_Borrow(TrigBoard):
	...
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		#只有当前要结束的回合的camp与自身camp相同的时候可以触发，于是即使有多个同类扳机也只有一个会触发。
		return self.entity.onBoard and camp == self.entity.camp
		
	def effect(self, signal, camp, subject, target, number, comment, choice=0):
		#Game的monsterSwitchSide方法会自行移除所有的此类扳机。
		self.entity.Game.monsterSwitchSide(self.entity, activity="Return")
...
class Trig_Corrupt(TrigHand):
	...
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		return self.entity.inHand and camp == self.entity.camp and number > self.entity.mana and subject.type != "Power"
		
	def effect(self, signal, camp, subject, target, number, comment, choice=0):
		card = self.entity
		newCard = self.corruptedType(card.Game, card.camp)
		try:
			...
			if newCard.type == "Monster":
				#Temporary attack changes on monsters are NOT included in attack_Enchant
				...
				attBuff, healthBuff = card.attack_Enchant - card.attack_0, card.health_max - card.health_0
				#Assume temporary attack changes applied on a monster won't carry over to the weapon
...
class Trig_DieatEndofTurn(TrigBoard):
	...
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		return self.entity.onBoard #Even if the current turn is not the monster's owner's turn
		
	def effect(self, signal, camp, subject, target, number, comment, choice=0):
		self.entity.Game.killMonster(None, self.entity)
		
		
class QuestTrigger(TrigBoard):
	def __init__(self, entity):
		self.blank_init(entity, ["MonsterBeenPlayed"])
...
class HasAura_toMonster:
	...
	def auraAppears(self):
		for monster in self.entity.Game.monstersonBoard(self.entity.camp):
			self.applies(monster)
		#Only need to handle monsters that appear. Them leaving/silenced will be handled by the Stat_Receiver object.
		for sig in self.signals:
			try: self.entity.Game.trigsBoard[self.entity.camp][sig].append(self)
			except: self.entity.Game.trigsBoard[self.entity.camp][sig] = [self]
			
	def auraDisappears(self):
		for monster, receiver in self.auraAffected[:]:
			receiver.effectClear()
		self.auraAffected = []
		for sig in self.signals:
			try: self.entity.Game.trigsBoard[self.entity.camp][sig].remove(self)
			except: pass
	...
	def createCopy(self, game):
		...
		if self not in game.copiedObjs: #这个光环没有被复制过
			...
			for monster, receiver in self.auraAffected:
				monsterCopy = monster.createCopy(game)
				index = monster.auraReceivers.index(receiver)
				receiverCopy = monsterCopy.auraReceivers[index]
				receiverCopy.source = Copy #补上这个receiver的source
				Copy.auraAffected.append((monsterCopy, receiverCopy))
			return Copy
		else:
			return game.copiedObjs[self]
...
class Stat_Receiver:
	...
	#Invoke when the affected monster is copied and because the aura_Dealer won't have reference to this copied receiver,
	#remove this copied receiver from copied receiver's auraReceivers.
	#Only monster selfCopy will invoke this
	...
	def selfCopy(self, recipient): #The recipient of the aura is the same monster when copying it.
		#Source won't change.
...
class StatAura_Others(HasAura_toMonster):
	def __init__(self, entity, attack, health):
		self.entity = entity
		self.attack, self.health = attack, health
		self.signals, self.auraAffected = ["MonsterAppears"], []
		
	#All monsters appearing on the same side will be subject to the buffAura.
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
		return self.entity.onBoard and subject.camp == self.entity.camp and subject != self.entity and self.applicable(subject)
...
#def __init__(self, monster, func, attack, health):
class StatAura_Adjacent(HasAura_toMonster):
	def __init__(self, entity, attack, health):
		self.entity = entity
		self.attack, self.health = attack, health
		self.signals, self.auraAffected = ["MonsterAppears", "MonsterDisappears"], []
	#Monsters appearing/disappearing will let the monster reevaluate the aura.
	def canTrig(self, signal, camp, subject, target, number, comment, choice=0):
	...
	def effect(self, signal, camp, subject, target, number, comment, choice=0):
		...
		for monster, receiver in self.auraAffected[:]:
			receiver.effectClear()
		#Find adjacent monsters to self.entity, then try to register them.
		for monster in self.entity.Game.neighbors2(self.entity)[0]:
			self.applies(monster)
	...
	def auraAppears(self):
		...
		for monster in game.neighbors2(self.entity)[0]:
			self.applies(monster)
			
		#Only need to handle monsters that appear. Them leaving/silenced will be handled by the Stat_Receiver object.
		for sig in self.signals: #随从离场时会自己清除自己的光环
			try: game.trigsBoard[self.entity.camp][sig].append(self)
			except: game.trigsBoard[self.entity.camp][sig] = [self]
			
	def selfCopy(self, recipient): #The recipient is the monster that deals the Aura.
		return type(self)(recipient, self.attack, self.health)
	#可以通过HasAura_toMonster的createCopy方法复制
	
	
class StatAura_Enrage(HasAura_toMonster):
	...
	def auraAppears(self):
		monster = self.entity
		try: monster.Game.trigsBoard[monster.camp]["MonsterStatCheck"].append(self)
		except: monster.Game.trigsBoard[monster.camp]["MonsterStatCheck"] = [self]
		if monster.onBoard and monster.health < monster.health_max and not self.on:
			self.on = True
			self.applies(monster)
			
	def auraDisappears(self):
		self.on = False
		try: self.entity.Game.trigsBoard[self.entity.camp]["MonsterStatCheck"].remove(self)
		except: pass
		for monster, receiver in self.auraAffected[:]:
			receiver.effectClear()
		self.auraAffected = []
		
	#To be overridden by other more unique enrage monsters
	...
	def effect(self, signal, camp, subject, target, number, comment, choice=0):
		monster = self.entity
		if monster.health < monster.health_max and not self.on:
			self.on = True
			self.applies(monster)
		elif monster.health >= monster.health_max and self.on:
			self.on = False
			for monster, receiver in self.auraAffected[:]:
				receiver.effectClear()
				
	def selfCopy(self, recipient): #The recipientMonster is the entity that deals the Aura.
		#func that checks if subject is applicable will be the new copy's function
		return type(self)(recipient, self.attack)
	#激怒的光环仍然可以通过HasAura_toMonster的createCopy复制
...
class EffectAura(HasAura_toMonster):
	def __init__(self, entity, keyWord):
		self.entity = entity
		self.keyWord = keyWord
		self.signals, self.auraAffected = ["MonsterAppears"], [] #List of (receiver, receiver)
	...
	#关键字光环可以通过HasAura_toMonster的createCopy方法复制
...
class ManaMod:
	...
	def applies(self):
		...
		if self.card in self.card.Game.Hand_Deck.hands[self.card.camp] or self.card in self.card.Game.Hand_Deck.decks[self.card.camp]:
...
class ManaAura:
	...
	def auraAppears(self):
		...
		try: game.trigsBoard[self.entity.camp]["CardEntersHand"].append(self)
		except: game.trigsBoard[self.entity.camp]["CardEntersHand"] = [self]
	...
	def auraDisappears(self):
		...
		self.auraAffected = []
		try: monster.Game.trigsBoard[monster.camp]["CardEntersHand"].remove(self)
		except: pass
		monster.Game.Manas.calcMana_All()
...
class TempManaEffect:
	def __init__(self, Game, camp, changeby=0, changeto=-1):
		self.blank_init(Game, camp, changeby, changeto)
		
	def blank_init(self, Game, camp, changeby, changeto):
	...
	def auraAppears(self):
		...
		for card in game.Hand_Deck.hands[2]: self.applies(card)
		for sig in self.signals:
			try: game.trigsBoard[self.camp][sig].append(self)
			except: game.trigsBoard[self.camp][sig] = [self]
		game.trigAuras[self.camp].append(self)
		game.Manas.calcMana_All()
		
	def auraDisappears(self):
		game = self.Game
		for monster, manaMod in self.auraAffected[:]:
			manaMod.getsRemoved()
		self.auraAffected = []
		try: game.Manas.CardAuras.remove(self)
		except: pass
		for sig in self.signals:
			try: game.trigsBoard[self.camp][sig].remove(self)
			except: pass
		try: game.trigAuras[self.camp].remove(self)
		except: pass
		game.Manas.calcMana_All()
		
	def selfCopy(self, game):
		return type(self)(game, self.camp)
...
class ManaAura_1UsageEachTurn: #For Pint-sized Summoner, Kalecgos, etc
	...
	def auraAppears(self):
		game, camp = self.entity.Game, self.entity.camp
		if game.turn == camp and game.Counters.numMonstersPlayedThisTurn[camp] < 1:
			self.aura = TempManaEffect(game, camp)
			self.aura.auraAppears()
		try: game.trigsBoard[camp]["TurnStarts"].append(self)
		except: game.trigsBoard[camp]["TurnStarts"] = [self]
	...
	def auraDisappears(self):
		if self.aura:
			...
			self.aura = None
		try: self.entity.Game.trigsBoard[self.entity.camp]["TurnStarts"].remove(self)
		except: pass
...
class TempManaEffect_Power:
	...
	def trig(self, signal, camp, subject, target, number, comment, choice=0):
		if self.applicable(subject):
			...
			elif subject == self.Game.powers[self.camp]: #Mana Paid consumes the aura
				self.auraDisappears()
	...
	def auraDisappears(self):
		...
		for sig in self.signals:
			try: self.Game.trigsBoard[self.camp][sig].remove(self)
			except: pass
		self.Game.Manas.calcMana_Powers()
		
	def selfCopy(self, game):
		return type(self)(game, self.camp, self.changeby, self.changeto)
...
class ManaAura_Power:
	...
	def auraAppears(self):
		...
		self.applies(game.powers[1])
		self.applies(game.powers[2])
		try: game.trigsBoard[self.entity.camp]["HeroPowerAcquired"].append(self)
		except: game.trigsBoard[self.entity.camp]["HeroPowerAcquired"] = [self]
	...
	def auraDisappears(self):
		for power, manaMod in self.auraAffected[:]:
			manaMod.getsRemoved()
		self.auraAffected = []
		try: self.entity.Game.trigsBoard[self.entity.camp]["HeroPowerAcquired"].remove(self)
		except: pass
