...
def classforDiscover(initiator):
	Class = initiator.Game.heroes[initiator.camp].Class
...
class Card:
	...
	def whenDiscarded(self):
		pass
	"""Handle the target selection. All methods belong to monsters. Other types will define their own methods."""
	...
	def targetCorrect(self, target, choice=0):
		return (target.type == "Monster" or target.type == "Hero") and target.onBoard

	def selectablebyBattle(self, subject):
				#攻击目标必须是在场上的没有潜行的敌方随从或英雄
				#被攻击目标必须能够被选择(没有免疫，潜行和“不能被攻击”)的随从或者英雄
				#攻击方如果有无视嘲讽，则不用判定嘲讽问题
		if self.onBoard and self.camp != subject.camp and self.status["Temp Stealth"] < 1:
			if self.type == "Monster":
				return self.status["Immune"] + self.keyWords["Stealth"] + self.marks["Can't Be Attacked"] < 1 \
						and (self.keyWords["Taunt"] > 0 \
							or not any(monster.keyWords["Taunt"] > 0 and monster.status["Temp Stealth"] + monster.status["Immune"] + monster.keyWords["Stealth"] < 1 for monster in self.Game.monstersonBoard(self.camp))
							or self.Game.status[subject.camp]["Ignore Taunt"] > 0 or (subject.type == "Monster" and subject.marks["Ignore Taunt"]))
			elif self.type == "Hero":
				return self.Game.status[self.camp]["Immune"] + self.Game.status[self.camp]["Hero Can't Be Attacked"] < 1 \
						and (not any(monster.keyWords["Taunt"] > 0 and monster.status["Temp Stealth"] + monster.status["Immune"] + monster.keyWords["Stealth"] < 1 for monster in self.Game.monstersonBoard(self.camp))
							or self.Game.status[subject.camp]["Ignore Taunt"] > 0 or (subject.type == "Monster" and subject.marks["Ignore Taunt"]))
		return False
	...
	def canSelect(self, target):
		...
		for target in targets:
			...
			selectable = False
			if target.onBoard and targetType in ("Hero", "Monster", "Amulet"):
				if targetType == "Hero":
					#如果英雄是敌方英雄且目前有潜行，免疫，且“不可被敌方效果指定”，则无论什么指向性效果都无法进行选择
					if target.camp != self.camp and self.Game.status[target.camp]["Immune"] + target.status["Temp Stealth"] + target.marks["Enemy Effect Evasive"] > 0:
						pass
					#如果subject是法术或者英雄技能，则如果英雄有魔免，或者对方英雄有对对方的魔免，则无法进行选择
					elif (self.type == "Power" or self.type == "Spell") and self.Game.status[target.camp]["Evasive"] > 0:
						pass
					...
					#	and (target.marks["Evasive"] > 1 or (target.camp != self.camp and target.marks["Enemy Evasive"] > 1)))
					...
					#print("Is the target an evasive monster?", target.marks["Evasive"] > 0, target.camp != self.camp and target.marks["Enemy Evasive"] > 1)
					#如果目标随从是敌方的，且目前拥有免疫，潜行或者临时潜行，则无法进行选择
					if target.camp != self.camp and target.status["Immune"] + target.keyWords["Stealth"] + target.status["Temp Stealth"] + target.marks["Enemy Effect Evasive"] > 0:
						pass
					#如果subject是法术或者英雄技能，且目标随从有魔免，或者是一个有“对对方魔免”的对方随从，则无法进行选择
					elif (self.type == "Power" or self.type == "Spell") \
						and (target.marks["Evasive"] > 0 or (target.camp != self.camp and target.marks["Enemy Evasive"] > 0)):
						print("Evasive works on subject", self, "by", target)
						pass
					else: selectable = True
				elif targetType == "Amulet":
					if target.camp != self.camp and target.marks["Enemy Effect Evasive"] > 0:
						pass
			...
			#			 (
			#				(targetType == "Hero" and (target.camp == self.camp or self.Game.status[target.camp]["Immune"] + target.status["Temp Stealth"] + target.marks["Enemy Effect Evasive"] < 1) \
			#											and not ((self.type == "Power" or self.type == "Spell") and self.Game.status[target.camp]["Evasive"] > 1)) \
			#				#不能被法术或者英雄技能选择的随从是： 魔免随从 或者 是对敌方魔免且法术或英雄技能是敌方的
			#				or (targetType == "Monster" and (target.camp == self.camp or target.status["Immune"] + target.keyWords["Stealth"] + target.status["Temp Stealth"] + target.marks["Enemy Effect Evasive"] < 1) \
			#												and not ((self.type == "Power" or self.type == "Spell") and (target.marks["Evasive"] > 1 or (target.camp != self.camp and target.marks["Enemy Evasive"] > 1)))) \
			#				or (targetType == "Amulet" and (target.camp == self.camp or target.marks["Enemy Effect Evasive"] < 1) \
	...
	def selectableEnemyMonsterExists(self, choice=0):
		return any(monster.type == "Monster" and monster.onBoard and self.canSelect(monster) and self.targetCorrect(monster, choice) \
					for monster in self.Game.monsters[3-self.camp])

	def selectableFriendlyMonsterExists(self, choice=0):
		return any(monster.type == "Monster" and monster.onBoard and self.canSelect(monster) and self.targetCorrect(monster, choice) \
					for monster in self.Game.monsters[self.camp])

	def selectableEnemyAmuletExists(self, choice=0):
		return any(monster.type == "Amulet" and monster.onBoard and self.canSelect(monster) and self.targetCorrect(monster, choice) \
					for monster in self.Game.monsters[3-self.camp])
	...
	def selectableMonsterExists(self, choice=0):
		return self.selectableFriendlyMonsterExists(choice) or self.selectableEnemyMonsterExists(choice)

	def selectableEnemyExists(self, choice=0):
		hero = self.Game.heroes[3-self.camp]
		return (self.canSelect(hero) and self.targetCorrect(hero, choice)) or self.selectableEnemyMonsterExists(choice)

	#There is always a selectable friendly character -- hero.
	def selectableFriendlyExists(self, choice=0): #For monster battlecries, the friendly hero is always selectable
		hero = self.Game.heroes[self.camp]
		return (self.type != "Spell" or self.type != "Power") \
				or (self.canSelect(hero) and self.targetCorrect(hero, choice) or self.selectableFriendlyMonsterExists(choice))
	...
	#For targeting battlecries(Monsters/Weapons)
	def targetExists(self, choice=0):
		return True

	def selectionLegit(self, target, choice=0):
		#抉择牌在有全选光环时，选项自动更正为一个负数。
		choice -= 3 * (self.need2Choose() and self.Game.status[self.camp]["Choose Both"] > 0)
		if target: #指明了目标
			...
			return not self.needTarget(choice) or ((self.type == "Monster" or self.type == "Amulet") and not (self.needTarget(choice) and self.targetExists(choice)))

	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		return target

	def countSpellDamage(self):
		return self.Game.status[self.camp]["Spell Damage"] \
				+ sum(monster.keyWords["Spell Damage"] or (monster.keyWords["Nature Spell Damage"] and self.school == "Nature")
					  for monster in self.Game.monsters[self.camp])
	...
	"""Handle the card doing battle(Monster and Hero)"""
	...
	def attacks(self, target, useAttChance=True):
		...
		if self.type == "Monster" and self.keyWords["Stealth"] > 0:
		...
		weapon = game.availableWeapon(self.camp)
		dmgDealer_att = weapon if self.type == "Hero" and weapon and self.camp == game.turn else self
		#如果被攻击者是英雄，且装备着当前回合打开着的武器，则将攻击目标造成的伤害来源视为那把武器。
		weapon = game.availableWeapon(target.camp)
		dmgDealer_def = weapon if target.type == "Hero" and weapon and target.camp == game.turn else target
		#Manipulate the health of the subject/target's health.
		#Only send out "MonsterTakesDmg" signal at this point.
		...
		if dmgDealer_att.type != "Hero" and dmgDealer_att.marks["Sweep"] > 0 and target.type == "Monster":
			neighbors = game.neighbors2(target)[0] #此时被攻击的随从一定是在场的，已经由Game.battleRequest保证。
			for monster in neighbors:
				dmgTaker = game.scapegoat4(monster, dmgDealer_att)
	...
	def getsFrozen(self):
		if self.type == "Monster" or self.type == "Hero":
			self.status["Frozen"] += 1
			if self.type == "Monster":
				self.Game.sendSignal("MonsterGetsFrozen", self.Game.turn, None, self, 0, "")
	...
	def getsImmune(self):
		if self.type == "Monster":
			...
			self.Game.status[self.camp]["Immune"] += 1
		if self.Game.GUI: self.Game.GUI.statusChangeAni(self)
	...
	#Handle Lifesteal of a card. Currently Monster/Weapon/Spell classs have this method.
	##法术因为有因为外界因素获得吸血的能力，所以有自己的tryLifesteal方法。
	def tryLifesteal(self, damage, damageType="None"):
		game = self.Game
		if self.keyWords["Lifesteal"] > 0 or (self.type == "Spell" and game.status[self.camp]["Spells Lifesteal"] > 0)\
				or (damageType == "Battle" and "Drain" in self.keyWords and self.keyWords["Drain"] > 0):
			heal = damage * (2 ** self.countHealDouble())
			if game.status[self.camp]["Heal to Damage"] > 0:
				#If the Lifesteal heal is converted to damage, then the obj to take the final
				#damage will not cause Lifesteal cycle.
				dmgTaker = self.Game.scapegoat4(game.heroes[self.camp])
				dmgTaker.takesDamage(self, heal, damageType="Ability")
			elif game.status[self.camp]["Lifesteal Damages Enemy"] > 0:
				dmgTaker = self.Game.scapegoat4(game.heroes[3-self.camp])
				dmgTaker.takesDamage(self, heal, damageType="Ability")
			else: #Heal is heal.
				game.heroes[self.camp].getsHealed(self, heal)
	...
	def dealsAOE(self, targets, damages):
		...
		for target, dmgActual in zip(dmgTakers, dmgsActual):
			game.sendSignal(target.type+"TakesDmg", self.camp, self, target, dmgActual, "")
			game.sendSignal(target.type+"TookDmg", self.camp, self, target, dmgActual, "")
	...
	def getsHealed(self, subject, heal, sendHealSignal=True):
		...
		if self.inHand or self.onBoard:#If the character is dead and removed already or in deck. Nothing happens.
			...
			if self.health < self.health_max:
				...
				game.Counters.healthRestoredThisGame[subject.camp] += healActual
				game.Counters.healthRestoredThisTurn[subject.camp] += healActual
				if self.type == "Monster":
					game.sendSignal("MonsterStatCheck", self.camp, None, self, 0, "")
				elif game.turn == self.camp:
					game.Counters.timesHeroChangedHealth_inOwnTurn[self.camp] += 1
					game.Counters.heroChangedHealthThisTurn[self.camp] = True
					game.sendSignal("HeroChangedHealthinTurn", self.camp, None, None, 0, "")
				if sendHealSignal: #During AOE healing, the signals are delayed.
	...
	"""Handle the battle options for monsters and heroes."""
	#Will only be used to find a selectable attack target
	def findBattleTargets(self):
		side = 3 - self.camp
		if self.canAttack():
			targets = [monster for monster in self.Game.monstersonBoard(side) if self.canAttackTarget(monster)]
			indices = [monster.pos for monster in targets]
			wheres = ["Monster%d"%side] * len(indices)
	...
	def findTargets(self, comment="", choice=0):
		...
		targets = [monster for monster in game.monstersonBoard(1) if self.targetCorrect(monster, choice) and (not comment or self.canSelect(monster))]
		indices = [monster.pos for monster in targets]
		wheres = ["Monster1"] * len(indices)
		targets2 = [monster for monster in game.monstersonBoard(2) if self.targetCorrect(monster, choice) and (not comment or self.canSelect(monster))]
		targets += targets2
		indices += [monster.pos for monster in targets2]
		wheres += ["Monster2"] * len(targets2)
		for camp in range(1, 3):
			hero = game.heroes[camp]
			if self.targetCorrect(hero, choice) and (not comment or self.canSelect(hero)):
				targets.append(hero)
				indices.append(camp)
				wheres.append("Hero%d"%camp)
		if targets: return targets, indices, wheres
		else: return [None], [0], ['']

	#Monster has its own selfCopy() method.
	#For now, copying non-monster/weapon cards can only create copies that don't have any enchantments on it.
	#The mana of a card can be copied, though.
	def hardCopy(self, camp, creator=""):
		Copy = type(self)(self.Game, camp)
		for key, value in self.__dict__.items():
			...
			#用于auras，attfromAura，effectfromAura和manaMods等
				...
				#A monster can't refernece another monster. The attributes here must be like triggers/deathrattles
				Copy.__dict__[key] = value.selfCopy(Copy)
		Copy.camp, Copy.creator = camp, creator
		return Copy

	#给非随从牌用的，目前也没有复制场上武器的牌
	def selfCopy(self, camp, creator):
		Copy = self.hardCopy(camp)
...
class Dormant(Card):
	...
	def blank_init(self, Game, camp):
		self.Game, self.camp = Game, camp
	...
	def turnStarts(self, camp):
		pass

	def turnEnds(self, camp):
		pass
...
class Monster(Card):
	...
	index = "Vanilla~Neutral~2~2~2~Monster~~Vanilla~Uncollectible"
	requireTarget, keyWord, description = False, "", ""
	def __init__(self, Game, camp):
		self.blank_init(Game, camp)

	def reset(self, camp, isKnown=True): #如果一个随从被返回手牌或者死亡然后进入墓地，其上面的身材改变(buff/statReset)会被消除，但是保留其白字变化
	...
	def blank_init(self, Game, camp):
		...
		self.type, self.race = "Monster", cardType.race
		...
		# Some state of the monster represented by the marks
		...
		self.silenced = False  # This mark is for monster state change, such as enrage.
		self.appearResponse, self.disappearResponse, self.silenceResponse, self.returnResponse = [], [], [], []
		# self.seq records the number of the monster's appearance. The first monster on board has a sequence of 0
		self.seq, self.pos = -1, -2
		self.attTimes = self.attChances_base = self.attChances_extra = 0

		self.auras = {}
		self.options = []  # For Choose One monsters.
	...
	"""Handle the trigsBoard/inHand/inDeck of monsters based on its move"""
	def appears(self, firstTime=True):
		self.onBoard = True
		self.inHand = self.inDeck = self.dead = False
		self.enterBoardTurn = self.Game.numTurn
		self.mana = type(self).mana  # Restore the monster's mana to original value.
		...
		self.Game.sendSignal("MonsterAppears", self.camp, self, None, 0, comment=firstTime)

	def disappears(self, deathrattlesStayArmed=True, disappearResponse=True):  # The monster is about to leave board.
		self.onBoard = self.inHand = self.inDeck = False
		for aura in self.auras.values(): aura.auraDisappears()
		for trig in self.trigsBoard: trig.disconnect()
		...
		self.Game.sendSignal("MonsterDisappears", self.camp, None, self, 0, "")
	...
	def turnStarts(self, camp):
		for i in reversed(range(len(self.tempAttChanges))):  # Remove the temp attack changes.
			...
			if self.tempAttChanges[i][1] == "StartofTurn 2" and self.Game.turn == 2:
				...
				self.tempAttChanges.pop(i)
		if camp == self.camp:
			if self.onBoard: self.status["Temp Stealth"] = 0  # Only monsters on board lose Temp Stealth
	...
	# The monster is still frozen when played. And since it's not actionable, it won't defrost at the end of turn either.
	...
	def turnEnds(self, camp):
		...
		if camp == self.camp:
			...
			# The monster can only thaw itself at the end of its turn. Not during or outside its turn
			if self.onBoard and self.status["Frozen"] > 0:  # The monster can't defrost in hand.
	...
	def actionable(self):
		...
		return self.camp == self.Game.turn and \
	...
	def getsKeyword(self, keyWord, amount=1):
		if self.inDeck == False:  # and keyWord in self.keyWords.keys()
			...
			if self.onBoard:
				self.decideAttChances_base()
				if keyWord == "Charge":
					self.Game.sendSignal("MonsterChargeChanged", self.Game.turn, self, None, 0, "")
			if self.Game.GUI: self.Game.GUI.statusChangeAni(self)
	...
	def losesKeyword(self, keyWord):
		if self.onBoard or self.inHand:
			if keyWord == "Stealth":
				...
				self.Game.sendSignal("MonsterLosesDivineShield", self.Game.turn, None, self, 0, "")
			else:
	...
	# Whether the monster can select the attack target or not.
	def canAttack(self):
		# THE CHARGE/RUSH MONSTERS WILL GAIN ATTACKCHANCES WHEN THEY APPEAR
	...
	def canAttackTarget(self, target):
		return self.canAttack() and target.selectablebyBattle(self) and \
			   (target.type == "Monster" or (target.type == "Hero" and
		...
		if self.status["Immune"] < 1:  # 随从首先结算免疫和圣盾对于伤害的作用，然后进行预检测判定
			...
			if damage > 0:
				if self.keyWords["Divine Shield"] > 0:
					...
					game.sendSignal("FinalDmgonMonster?", 0, subject, self, damageHolder, "")
					...
					if ((subject.type == "Spell" and game.status[subject.camp]["Spells Poisonous"] > 0) or
						subject.keyWords["Poisonous"] > 0) and self.onBoard:
						self.dead = True
					# 在同时涉及多个角色的伤害处理中，受到的伤害暂不发送信号而之后统一进行扳机触发。
					if sendDmgSignal:
						game.sendSignal("MonsterTakesDmg", game.turn, subject, self, damage, "")
						game.sendSignal("MonsterTookDamage", game.turn, subject, self, damage, "")
					if subject.type == "Power":
						game.Counters.damageDealtbyHeroPower[subject.camp] += damage
					# 随从的激怒，根据血量和攻击的状态改变都在这里触发。
					self.Game.sendSignal("MonsterStatCheck", self.camp, None, self, 0, "")
			else:
				game.sendSignal("MonsterTakes0Dmg", game.turn, subject, self, 0, "")
				game.sendSignal("MonsterTook0Dmg", game.turn, subject, self, 0, "")
		else:
			damage = 0
		return damage

	def deathResolution(self, attackwhenDies, armedTrigs_WhenDies, armedTrigs_AfterDied):
		self.Game.sendSignal("MonsterDies", self.Game.turn, None, self, attackwhenDies, '', 0,
							 armedTrigs_WhenDies)
		...
		# The monster resets its own status. But it will record its current location.
		...
		# 移除随从注册了的亡语扳机
		for trig in self.deathrattles: trig.disconnect()
		self.Game.sendSignal("MonsterDied", self.Game.turn, None, self, 0, "", 0, armedTrigs_AfterDied)

	# MonsterDeathResolutionFinished
	...
	def magnetCombine(self, target):
		...
		if self.onBoard:
			...
			self.Game.removeMonsterorWeapon(self)
		else:  # The Magnetic monster is inHand
			self.Game.Hand_Deck.extractfromHand(self)

		# 在打出磁力随从时，需要得知其原有的关键字、身材和扳机等，需要进行复制，然后录入随从的磁力升级历史
		Copy = self.selfCopy(self.camp)  # Find the original keywords the monster has
		...
		self.Game.monsterPlayed = None

	# Specifically for battlecry resolution. Doesn't care if the target is in Stealth.
	def targetCorrect(self, target, choice=0):
		return (target.type == "Monster" or target.type == "Hero") and target.onBoard and target != self

	# Monsters that initiates discover or transforms self will be different.
	# For monster that transform before arriving on board, there's no need in setting its onBoard to be True.
	...
	def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
		...
		GUI = self.Game.GUI
		if GUI: GUI.boardZones[self.camp].draw(blockwhilePlaying=False)
		# 使用阶段
		...
		self.Game.sendSignal("MonsterPlayed", self.camp, self, target, mana, "", choice)
		# 召唤时步骤，触发“每当你召唤一个xx随从”的扳机.如鱼人招潮者等。
		self.Game.sendSignal("MonsterSummoned", self.camp, self, target, mana, "")
		# 过载结算
		if self.overload > 0:
			self.Game.Manas.overloadMana(self.overload, self.camp)
		...
		self.Game.gathertheDead()  # At this point, the monster might be removed/controlled by Illidan/Juggler combo.
		# 结算阶段
		if self.magnetic > 0:
			# 磁力相当于伪指向性战吼，一旦指定目标之后，不会被其他扳机改变
			# 磁力随从需要目标和自己都属于同一方,且磁力目标在场时才能触发。
			if magneticTarget and magneticTarget.onBoard and self.camp == magneticTarget.camp:
		...
		# 磁力随从没有战吼等入场特效，因而磁力结算不会引发死亡，不必进行死亡结算
			...
			if target and not isinstance(target, list):
				targetHolder = [target]
				self.Game.sendSignal("BattlecryTargetDecision", self.camp, self, targetHolder, 0, "", choice)
			...
			num, status = 1, self.Game.status[self.camp]
	...
	def countHealDouble(self):  # 随从和武器的治疗效果
		return sum(monster.marks["Heal x2"] > 0 for monster in self.Game.monsters[self.camp])
	...
	def buffDebuff(self, attackGain, healthGain, attRevertTime=''):
		if not self.inDeck and not self.dead:  # 只有随从在场上或者手牌中的时候可以接受buff。
			...
			else:  # Monsters can receive temp attack changes, too. And those will also vanish at the corresponding time point.
				self.tempAttChanges.append((attackGain, attRevertTime))
				self.statChange(attackGain, healthGain)
			if attackGain > 0 or healthGain > 0:
				self.Game.sendSignal("MonsterBuffed", self.camp, self, None, 0, "")
	...
	def statReset(self, newAttack=False, newHealth=False, attRevertTime=""):
		if not self.inDeck and not self.dead:
			...
			if newAttack != False:  # The monster's health is reset.
				self.tempAttChanges = []  # Clear the temp attack changes on the monster.
				if attRevertTime:
	...
	# 在原来的Game中创造一个Copy
	def selfCopy(self, camp, creator, attack=False, health=False, mana=False):
		Copy = self.hardCopy(camp)
	...
	def loseAbility(self):
		self.silenced = True
		for aura in self.auras.values(): aura.auraDisappears() #Clear the monster's auras
		self.auras = {}
		...
		for key, value in self.status.items():
			...
			# The monster only remember one Borrowed state, even if repetitively moved between two sides.
			if key == "Borrowed" and value > 0 and self.onBoard:
				self.Game.monsterSwitchSide(self, activity="Return")
			self.status[key] = 0
...
class Spell(Card):
	...
	def reset(self, camp, isKnown=True):
		creator, possi = self.creator, type(self) if isKnown else self.possi
		btn, x, y, z = self.btn, self.x, self.y, self.z
		self.__init__(self.Game, camp)
	...
	def cast(self, target=None, comment="", preferedTarget=None):
		...
		repeatTimes = 2 if curGame.status[self.camp]["Spells x2"] > 0 else 1
		...
		if curGame.mode == 0:
			if curGame.guides:
				...
				else: choice = -1 if curGame.status[self.camp]["Choose Both"] else nprandint(len(self.options))
				...
				if target: i, where = target.pos, target.type+str(target.camp)
				else: i, where = 0, ''
		...
		for i in range(repeatTimes):
			if self.overload > 0: curGame.Manas.overloadMana(self.overload, self.camp)
			if self.twinSpell > 0: #如果不是从手牌中打出，则不会把双生法术牌置入原来的位置
				curGame.Hand_Deck.addCardtoHand(self.twinSpellCopy, self.camp, byType=True,
		...
		if GUI: GUI.eraseOffBoardTrig(self.camp)
		#使用后步骤，但是此时的扳机只会触发星界密使和风潮的状态移除。这个信号不是“使用一张xx牌之后”的扳机。
		curGame.sendSignal("SpellBeenCast", self.camp, self, target, 0, "byOthers", choice=0)

	#泽蒂摩加风潮，当对泽蒂摩使用Mutate之后，Mutate会连续两次都进化3个随从
	#泽蒂摩是在法术开始结算之前打上标记,而非在连续两次之间进行判定。
	#comment = "InvokedbyAI", "Branching-i", ""
	def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
		game, camp, GUI = self.Game, self.camp, self.Game.GUI
		#使用阶段
		#判定该法术是否会因为风潮的光环存在而释放两次。发现的子游戏中不会两次触发，直接跳过
		repeatTimes = 2 if game.status[camp]["Spells x2"] > 0 else 1
		if GUI:
			GUI.showOffBoardTrig(self, alsoDisplayCard=True)
			GUI.wait(500)
		#使用时步骤，触发伊利丹和紫罗兰老师等“每当你使用一张xx牌”的扳机
		game.sendSignal("SpellPlayed", camp, self, target if not isinstance(target, list) else None, mana, "", choice)
		game.sendSignal("Spellboost", camp, self, None, mana, "", choice)
		#获得过载和双生法术牌。
		if self.overload > 0: game.Manas.overloadMana(self.overload, camp)
		if self.twinSpell > 0:
			game.Hand_Deck.addCardtoHand(self.twinSpellCopy, camp, byType=True, pos=posinHand,
													creator=type(self), possi=(self.twinSpellCopy,))
		#使用阶段结束，进行死亡结算。不处理胜负裁定。
		game.gathertheDead() #At this point, the monster might be removed/controlled by Illidan/Juggler combo.
		#进行目标的随机选择和扰咒术的目标改向判定。
		if target:
			holder = [target]
			game.sendSignal("SpellTargetDecision", camp, self, holder, 0, choice)
		...
		if target and target.camp == camp:
			game.Counters.spellsonFriendliesThisGame[camp].append(self.index)
		...
		sweep = game.status[camp]["Spells Sweep"]
		...
		for i in range(repeatTimes):
			if i == 1: #第二次施放时照常获得过载和双生法术牌。
				...
				if self.twinSpell > 0:
					game.Hand_Deck.addCardtoHand(self.twinSpellCopy, camp, byType=True, pos=posinHand,
													creator=type(self), possi=(self.twinSpellCopy,))
			#When the target is an onBoard monster, Zentimo is still onBoard and has adjacent monsters next to it.
			if target and target.type == "Monster" and target.onBoard and sweep > 0 and game.neighbors2(target)[0]:
				...
				for monster in neighbors: #对相邻的随从也释放该法术。
					monster.history["Spells Cast on This"].append(self.index)
					self.whenEffective(monster, comment, choice, posinHand)
			else: #The target isn't monster or Zentimo can't apply to the situation. Be the target hero, monster onBoard or inDeck or None.
				#如果目标不为空而且是在场上的随从，则这个随从的历史记录中会添加此法术的index。
				if target and (target.type == "Monster" or target.type == "Amulet") and target.onBoard:
					target.history["Spells Cast on This"].append(self.index)
				target = self.whenEffective(target, comment, choice, posinHand)
	...
	#完成阶段：
		...
		#	echoCard = type(monster)(self, game.turn)
		#	trig = Trig_Echo(echoCard)
		#	echoCard.trigsHand.append(trig)
		#	game.Hand_Deck.addCardtoHand(echoCard, camp)
		if GUI: GUI.eraseOffBoardTrig(camp)
	...
	def countDamageDouble(self):
		return sum(monster.marks["Spell Heal&Dmg x2"] > 0 for monster in self.Game.monsters[self.camp])

	def countHealDouble(self):
		return sum(monster.marks["Heal x2"] > 0 or monster.marks["Spell Heal&Dmg x2"] > 0 for monster in self.Game.monsters[self.camp])
...
class Secret(Spell):
	...
	def blank_init(self, Game, camp):
		super().blank_init(Game, camp)
		self.dummyTrigs = []
		
	def available(self):
		return self.Game.Secrets.areaNotFull(self.camp) and not self.Game.Secrets.sameSecretExists(self, self.camp)
	...
	def cast(self, target=None, comment="byOthers"):
		...
		if self.Game.GUI: self.Game.GUI.eraseOffBoardTrig(self.camp)
		self.Game.sendSignal("SpellBeenCast", self.camp, self, None, 0, "byOthers")

	def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
		if self.Game.GUI:
			self.Game.GUI.showOffBoardTrig(self, alsoDisplayCard=True, notSecretBeingPlayed=False)
			self.Game.GUI.wait(500)
		self.Game.sendSignal("SpellPlayed", self.camp, self, None, mana, "", choice)
		self.Game.sendSignal("Spellboost", self.camp, self, None, mana, "", choice)
		self.Game.gathertheDead()  # At this point, the monster might be removed/controlled by Illidan/Juggler combo.
		self.whenEffective(None, '', choice, posinHand)
		# There is no need for another round of death resolution.
		if self.Game.GUI: self.Game.GUI.eraseOffBoardTrig(self.camp)
		self.Game.sendSignal("SpellBeenCast", self.camp, self, None, 0, "")

	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		secretHD = self.Game.Secrets
		if secretHD.areaNotFull(self.camp) and not secretHD.sameSecretExists(self, self.camp):
			secretHD.secrets[self.camp].append(self)
...
class Quest(Spell):
	...
	def available(self):
		...
		if secretZone.areaNotFull(self.camp):
			if self.description.startswith("Sidequest"):
				return all(quest.name != self.name for quest in secretZone.sideQuests[self.camp])
			else: return secretZone.mainQuests[self.camp] != []
		return False
	...
	def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
		...
		if self.Game.GUI: self.Game.GUI.eraseOffBoardTrig(self.camp)
		self.Game.sendSignal("SpellBeenCast", self.camp, self, None, 0, "")
		self.Game.Counters.hasPlayedQuestThisGame[self.camp] = True

	def whenEffective(self, target=None, comment="", choice=0, posinHand=-2):
		secretZone = self.Game.Secrets
		if secretZone.areaNotFull(self.camp):
			if self.description.startswith("Sidequest") \
				and all(quest.name != self.name for quest in secretZone.sideQuests[self.camp]):
				secretZone.sideQuests[self.camp].append(self)
				for trig in self.trigsBoard: trig.connect()
			else:  # The quest is a main quest
				if not secretZone.mainQuests[self.camp]:
					secretZone.mainQuests[self.camp].append(self)
					for trig in self.trigsBoard: trig.connect()
		return None
...
class HeroPower(Card):
	...
	def appears(self):
		...
		for trig in self.trigsBoard:
			trig.connect()
		self.Game.sendSignal("HeroPowerAcquired", self.camp, self, None, 0, "")
	...
	def replaceHeroPower(self):
		powers, camp = self.Game.powers, self.camp
		if powers[camp]:
			powers[camp].disappears()
			powers[camp] = None
		powers[camp] = self
		self.appears()
		
	def chancesUsedUp(self):
		return self.Game.status[self.camp]["Power Chance Inf"] < 1 \
				and self.heroPowerTimes >= (1 + (self.Game.status[self.camp]["Power Chance 2"] > 0) )
	...
	def use(self, target=None, choice=0):
		...
		if self.Game.Manas.affordable(self) and self.available() and self.selectionLegit(target, choice):
			...
			#支付费用，清除费用状态。
			subIndex, subWhere = self.camp, "Power"
			if target: tarIndex, tarWhere = target.pos, target.type+str(target.camp)
			...
			targetHolder = [target]
			self.Game.sendSignal("HeroPowerTargetDecision", self.camp, self, targetHolder, 0, "", choice)
			...
			monstersKilled = 0
			if target and target.type == "Monster" and self.Game.status[self.camp]["Power Sweep"] > 0:
				targets = self.Game.neighbors2(target)[0]
				monstersKilled += self.effect(target, choice)
				if targets != []:
					for monster in targets: monstersKilled += self.effect(monster, choice)
			else: monstersKilled += self.effect(target, choice)

			#结算阶段结束，处理死亡，此时尚不进行胜负判定。
			#假设触发英雄技能消灭随从的扳机在死亡结算开始之前进行结算。（可能不对，但是相对比较符合逻辑。）
			if monstersKilled > 0:
				self.Game.sendSignal("HeroPowerKilledMonster", self.Game.turn, self, None, monstersKilled, "")
			self.Game.gathertheDead()
			self.heroPowerTimes += 1
			#激励阶段，触发“每当你使用一次英雄技能”的扳机，如激励，虚空形态的技能刷新等。
			self.Game.Counters.powerUsedThisTurn += 1
			self.Game.sendSignal("HeroUsedAbility", self.camp, self, target, self.mana, "", choice)
	...
	def countDamageDouble(self):
		return sum(monster.marks["Power Heal&Dmg x2"] > 0 for monster in self.Game.monsters[self.camp])

	def countHealDouble(self):
		return sum(monster.marks["Heal x2"] > 0 or monster.marks["Power Heal&Dmg x2"] > 0 for monster in self.Game.monsters[self.camp])
...
class Hero(Card):
	...
	def blank_init(self, Game, camp):
		...
		self.dead = False
		self.pos = self.camp
		self.heroPower = cardType.heroPower(self.Game, self.camp) if cardType.heroPower else None
	...
	def actionable(self):
		return self.camp == self.Game.turn
		#不考虑冻结、零攻和自身有不能攻击的debuff的情况。

	def decideAttChances_base(self):
		weapon = self.Game.availableWeapon(self.camp)
		self.attChances_base = 2 if self.keyWords["Windfury"] > 0 or (weapon and weapon.keyWords["Windfury"] > 0) else 1

	def turnStarts(self, camp):
		if camp == self.camp:
			self.status["Temp Stealth"] = 0
			if self.Game.status[self.camp]["Immune2NextTurn"] > 0:
				self.Game.status[self.camp]["Immune"] -= self.Game.status[self.camp]["Immune2NextTurn"]
				self.Game.status[self.camp]["Immune"] = max(0, self.Game.status[self.camp]["Immune"])
				self.Game.status[self.camp]["Immune2NextTurn"] = 0
			if self.Game.status[self.camp]["Evasive2NextTurn"] > 0:
				self.Game.status[self.camp]["Evasive"] -= self.Game.status[self.camp]["Evasive2NextTurn"]
				self.Game.status[self.camp]["Evasive2NextTurn"] = 0
	...
	def turnEnds(self, camp):
		if self.Game.status[self.camp]["ImmuneThisTurn"] > 0:
			self.Game.status[self.camp]["Immune"] -= self.Game.status[self.camp]["ImmuneThisTurn"]
			self.Game.status[self.camp]["Immune"] = max(0, self.Game.status[self.camp]["Immune"])
			self.Game.status[self.camp]["ImmuneThisTurn"] = 0
		#一个角色只有在自己的回合结束时有剩余的攻击机会才能解冻
		if camp == self.camp and self.status["Frozen"] > 0 and self.attChances_base + self.attChances_extra > self.attTimes:
			self.status["Frozen"] = 0
	...
	def calc_Attack(self):
		if self.Game.turn == self.camp:
			weapon = self.Game.availableWeapon(self.camp)
			self.attack = self.attack_bare + max(0, weapon.attack) if weapon else self.attack_bare
		else:
			self.attack = self.attack_bare
		if self.Game.GUI: self.Game.GUI.statChangeAni(self)
		self.Game.sendSignal("HeroAttCalc", self.camp, self, None, 0, "")

	def gainsArmor(self, armor):
		if armor > 0:
			if self.Game.GUI: self.Game.GUI.statChangeAni(self)
			self.armor += armor
			self.Game.sendSignal("ArmorGained", self.camp, self, None, armor, "")

	def losesArmor(self, armor, all=False):
		if all:
			ownArmor = self.armor
			self.armor = 0
			if ownArmor: self.Game.sendSignal("ArmorLost", self.camp, self, None, ownArmor, "")
		else:
			self.armor -= armor
			self.Game.sendSignal("ArmorLost", self.camp, self, None, armor, "")
		if self.Game.GUI: self.Game.GUI.statChangeAni(self)
	...
	def takesDamage(self, subject, damage, sendDmgSignal=True, damageType="None"):
		game = self.Game
		if game.status[self.camp]["Immune"] < 1:  # 随从首先结算免疫和圣盾对于伤害的作用，然后进行预检测判定
			if "Next Damage 0" in self.marks and self.marks["Next Damage 0"] > 0:
			...
			if damage > 0:
				damageHolder = [damage]
				game.sendSignal("FinalDmgonHero?", self.camp, subject, self, damageHolder, "")
				damage = damageHolder[0]
				if damage > 0:
					...
					if self.Game.GUI: self.Game.GUI.statChangeAni(self, num2=-damage)
					game.Counters.dmgonHero_inOppoTurn[self.camp] += damage
					if sendDmgSignal:
						game.sendSignal("HeroTakesDmg", game.turn, subject, self, damage, "")
						game.sendSignal("HeroTookDmg", game.turn, subject, self, damage, "")
					if game.turn == self.camp:
						game.Counters.timesHeroChangedHealth_inOwnTurn[self.camp] += 1
						game.Counters.timesHeroTookDamage_inOwnTurn[self.camp] += 1
						game.Counters.heroChangedHealthThisTurn[self.camp] = True
						game.sendSignal("HeroChangedHealthinTurn", self.camp, None, None, 0, "")
			else:
	...
	def healthReset(self, health, health_max=False):
		...
		if self.Game.GUI: self.Game.GUI.statChangeAni(self)
		if healthChanged and self.Game.turn == self.camp:
			self.Game.Counters.timesHeroChangedHealth_inOwnTurn[self.camp] += 1
			self.Game.Counters.heroChangedHealthThisTurn[self.camp] = True
			self.Game.sendSignal("HeroChangedHealthinTurn", self.camp, None, None, 0, "")
	...
	#使用阶段
		...
		game, camp = self.Game, self.camp
		oldHero = game.heroes[camp]
	...
	#结算阶段
		...
		if game.status[camp]["Battlecry x2"] > 0:
			self.whenEffective(None, "", choice)
		self.whenEffective(None, "", choice)
		if self.weapon: #如果英雄牌本身带有武器，如迦拉克隆等。则装备那把武器
			game.equipWeapon(self.weapon(game, camp))
		self.calc_Attack()
		self.decideAttChances_base()
		#结算阶段结束，处理死亡，此时尚不进行胜负判定。
		game.gathertheDead()
	...
	def replaceHero(self, fromHeroCard=False):
		...
		game, camp = self.Game, self.camp
		healthChanged = game.heroes[camp].health == self.health #目前只有大王和拉格纳罗斯会改变英雄的血量
		self.onBoard, self.pos, self.attTimes = True, camp, game.heroes[camp].attTimes
		...
		game.sendSignal("HeroReplaced", camp, self, None, 0, "")
		if healthChanged and game.turn == camp:
			game.Counters.timesHeroChangedHealth_inOwnTurn[camp] += 1
			game.Counters.heroChangedHealthThisTurn[camp] = True
			game.sendSignal("HeroChangedHealthinTurn", camp, None, None, 0, "")
...
class Weapon(Card):
	...
	def setasNewWeapon(self):
		...
		self.seq = len(curGame.monsters[1]) + len(curGame.monsters[2]) + len(curGame.weapons[1]) + len(curGame.weapons[2]) - 1
		curGame.heroes[self.camp].calc_Attack()
		curGame.heroes[self.camp].decideAttChances_base()
		curGame.sendSignal("WeaponEquipped", self.camp, self, None, 0, "")
	...
	def destroyed(self):
		if self.onBoard:  # 只有装备着的武器才会触发，以防连续触发。
			if self.keyWords["Windfury"] > 0:
				self.Game.heroes[self.camp].decideAttChances_base()
			self.onBoard, self.dead = False, True
			self.Game.heroes[self.camp].attack = self.Game.heroes[self.camp].attack_bare
			self.Game.sendSignal("HeroAttCalc", self.camp, self, None, 0, "")
			# 移除武器对应的场上扳机，亡语扳机在deathrattles中保存
			for trig in self.trigsBoard: trig.disconnect()
			for aura in self.auras.values(): aura.auraDisappears()
	# self.Game.sendSignal("WeaponRemoved", self.camp, self, None, 0, "")

	def deathResolution(self, attackwhenDies, armedTrigs_WhenDies, armedTrigs_AfterDied):
		# 除了武器亡语以外，目前只有一个应对武器被摧毁的扳机，即冰封王座的Grave Shambler
		self.Game.sendSignal("WeaponDestroyed", self.camp, None, self, 0, "", armedTrigs_WhenDies)
		self.Game.sendSignal("WeaponRemoved", self.camp, None, self, 0, "")
		for trig in self.deathrattles: trig.disconnect()
	...
	def disappears(self):
		if self.onBoard:  # 只有装备着的武器才会触发，以防连续触发。
			if self.keyWords["Windfury"] > 0:
				self.Game.heroes[self.camp].decideAttChances_base()
			self.onBoard = False
			self.Game.heroes[self.camp].calc_Attack()
			# 移除武器对应的场上扳机，亡语扳机在deathrattles中保存
			for trig in self.trigsBoard: trig.disconnect()
			for aura in self.auras.values(): aura.auraDisappears()
			self.Game.sendSignal("WeaponRemoved", self.camp, self, None, 0, "")
	...
	def gainStat(self, attack, durability):
		...
		self.Game.sendSignal("WeaponAttChanges", self.camp, None, None, 0, "")
	...
	def played(self, target=None, choice=0, mana=0, posinHand=-2, comment=""):
		...
		# 根据市长的存在情况来决定随机目标。
		targetHolder = [target]
		self.Game.sendSignal("BattlecryTargetDecision", self.camp, self, targetHolder, 0, "", choice=0)
		if target != targetHolder[0] and self.Game.GUI:
			target = targetHolder[0]
			self.Game.GUI.target = target
		...
		self.Game.GUI.wait(400)
		if self.Game.status[self.camp]["Battlecry x2"] > 0:
			# 对方武器而言没有必要返回主体对象，但是当战吼被沙德沃克调用的时候，需要返回。
			target = self.whenEffective(target, "", choice, posinHand)
		target = self.whenEffective(target, "", choice, posinHand)
		# 消灭旧武器，并将列表前方的武器全部移除。
		for weapon in self.Game.weapons[self.camp]:
			if weapon != self:
