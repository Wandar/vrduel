...
gameStatusDict = {"Immune": "你的英雄免疫", "Immune2NextTurn": "你的英雄免疫直到你的下个回合", "ImmuneThisTurn": "你的英雄在本回合免疫",
				...
				"Power Can Target Monsters": "你的英雄技能可以以随从为目标",
				...
				"Monsters Can't Be Frozen": "你的随从无法被冻结", #Living Dragonbreath prevents monsters from being Frozen
...
class Game:
	def __init__(self, GUI=None):
		self.mainPlayercamp = random.randint(1,2)
		self.GUI = GUI

	def initialize(self):
		self.heroes = {1:Illidan(self, 1), 2:Anduin(self, 2)} #player
		self.powers = {1:self.heroes[1].heroPower, 2:self.heroes[2].heroPower} #player power
		self.heroes[1].onBoard, self.heroes[2].onBoard = True, True
		#Multipole weapons can coexitst at monsters in lists. The newly equipped weapons are added to the lists
		self.monsters, self.weapons = {1:[], 2:[]}, {1:[], 2:[]}  #monsters weapons
		self.options, self.mulligans = [], {1:[], 2:[]}  #第二个是UI
		...
		self.monsterPlayed = None #Used for target change induced by triggers such Mayor Noggenfogger and Spell Bender.
	...
	def initialize_Details(self, cardPool, ClassCards, NeutralCards, MonstersofCost, RNGPools, hero1, hero2, deck1=[], deck2=[]):
		hero1 = hero1(self, 1)
		hero1.onBoard = True
		self.heroes[1] = hero1
		...
		self.MonstersofCost, self.RNGPools = MonstersofCost, RNGPools
		from Hand import Hand_Deck
		self.Hand_Deck = Hand_Deck(self, deck1, deck2)
		self.Hand_Deck.initialize()
		
	def monstersAlive(self, camp, target=None): #if target is not None, return all living monsters except the target
		if target: return [monster for monster in self.monsters[camp] if monster.type == "Monster" \
							and monster != target and monster.onBoard and (not monster.dead or monster.marks["Can't Break"] > 0) and monster.health > 0]
		else: return [monster for monster in self.monsters[camp] if monster.type == "Monster" and monster.onBoard and (not monster.dead or monster.marks["Can't Break"] > 0) and monster.health > 0]


	def monstersonBoard(self, camp, target=None): #if target is not None, return all onBoard monsters except the target
		if target: return [monster for monster in self.monsters[camp] if monster.type == "Monster" and monster.onBoard and monster != target]
		else: return [monster for monster in self.monsters[camp] if monster.type == "Monster" and monster.onBoard]

	def amuletsonBoard(self, camp, target=None):
		if target: return [amulet for amulet in self.monsters[camp] if amulet.type == "Amulet" and amulet.onBoard and amulet != target]
		else: return [amulet for amulet in self.monsters[camp] if amulet.type == "Amulet" and amulet.onBoard]

	def monstersandAmuletsonBoard(self, camp, target=None):
		if target: return [amulet for amulet in self.monsters[camp] if amulet.type != "Dormant" and amulet.onBoard and amulet != target]
		else: return [amulet for amulet in self.monsters[camp] if amulet.type != "Dormant" and amulet.onBoard]

	def earthsonBoard(self, camp, target=None):
		if target: return [amulet for amulet in self.monsters[camp] if amulet.type == "Amulet" and amulet.onBoard and "Earth Sigil" in amulet.race and amulet != target]
		else: return [amulet for amulet in self.monsters[camp] if amulet.type == "Amulet" and amulet.onBoard and "Earth Sigil" in amulet.race]

	def neighbors2(self, target, countDormants=False):
		targets, camp, pos, i = [], target.camp, target.pos, 0
		while pos > 0:
			pos -= 1
			obj_onLeft = self.monsters[camp][pos]
			if not countDormants and obj_onLeft.type != "Monster": break #If Dormants aren't considered as adjacent entities, they block the search
			elif obj_onLeft.onBoard: #If the monster is not onBoard, skip it; if on board, count it.
				targets.append(obj_onLeft)
				i -= 1
				break
		pos = target.pos
		boardSize = len(self.monsters[camp])
		while pos < boardSize - 1:
			pos += 1
			obj_onRight = self.monsters[camp][pos]
			if not countDormants and obj_onRight.type != "Monster": break
	...
	def charsAlive(self, camp, target=None):
		hero = self.heroes[camp]
		objs = [obj for obj in self.monsters[camp] if obj.type == "Monster" and obj != target and obj.onBoard and not obj.dead and obj.health > 0]
	...
	def playAmulet(self, amulet, target, position, choice=0, comment=""):
		camp, canPlayAmulet = amulet.camp, False
		if self.Manas.affordable(amulet) and self.space(camp) and amulet.selectionLegit(target, choice):
			...
			subIndex, subWhere = self.Hand_Deck.hands[amulet.camp].index(amulet), "Hand%d" % amulet.camp
			if target: #因为护符是SV特有的卡牌类型，所以其目标选择一定是列表填充式的
				tarIndex, tarWhere = [], []
				for obj in target:
					if obj.onBoard:
						tarIndex.append(obj.pos)
						tarWhere.append(obj.type+str(obj.camp))
					else:
						tarIndex.append(self.Hand_Deck.hands[obj.camp].index(obj))
						tarWhere.append("Hand%d"%obj.camp)
			...
			# The new monster played will have the largest sequence.
			# 处理随从的位置的登场顺序。
			amulet.seq = len(self.monsters[1]) + len(self.monsters[2]) + len(self.weapons[1]) + len(self.weapons[2])
			self.monsters[camp].insert(position + 100 * (position < 0), amulet)
			self.sortPos()
	...
	#Monsters to die will still count as a placeholder on board. Only monsters that have entered the tempDeads don't occupy space.
	def space(self, camp):
		#Monsters and Dormants both occupy space as long as they are on board.
		return 7 - 2 * (self.heroes[camp].Class in SVClasses) - sum(monster.onBoard for monster in self.monsters[camp])

	def playMonster(self, monster, target, position, choice=0, comment=""):
		camp, canPlayMonster = monster.camp, False
		#当场上没有空位且打出的随从不是一个有Accelerate的Shadowverse随从时，不能打出
		if self.Manas.affordable(monster) and (self.space(camp) or (monster.index.startswith("SV_") and monster.willAccelerate())) \
				and monster.selectionLegit(target, choice):
			#打出随从到所有结算完结为一个序列，序列完成之前不会进行胜负裁定。
			#打出随从产生的序列分为
				#1）使用阶段： 支付费用，随从进入战场（处理位置和刚刚召唤等），抉择变形类随从立刻提前变形，黑暗之主也在此时变形。
					#如果随从有回响，在此时决定其将在完成阶段结算回响
			...
			subIndex, subWhere = self.Hand_Deck.hands[monster.camp].index(monster), "Hand%d"%monster.camp
			if target:
				if isinstance(target, list):
					...
					tarIndex, tarWhere = target.pos, target.type+str(target.camp)
			else: tarIndex, tarWhere = 0, ''
			monster, mana, posinHand = self.Hand_Deck.extractfromHand(monster, enemyCanSee=True, animate=False)
			print("Play monster extract from hand finished")
			#如果打出的随从是SV中的爆能强化，激奏和结晶随从，则它们会返回自己的真正要打出的牌以及对应的费用
			try: monster, mana = monster.becomeswhenPlayed(choice)
			except: pass #如果随从没有爆能强化等，则无事发生。
			self.Manas.payManaCost(monster, mana) #海魔钉刺者，古加尔和血色绽放的伤害生效。
			#需要根据变形成的随从来进行不同的执行
			if monster.type == "Spell": #Shadowverse Accelerate monster might become spell when played
				self.monsterPlayed, spell = None, monster
				...
				if not spellHolder:
					...
					self.Counters.shadows[spell.camp] += 1
			else: #Normal or Enhance X or Crystallize X monster played
				typewhenPlayed = monster.type
				monsterIndex = monster.index
				monster.seq = len(self.monsters[1]) + len(self.monsters[2]) + len(self.weapons[1]) + len(self.weapons[2])
				self.monsters[camp].insert(position+100*(position < 0), monster)
				self.sortPos()
				if typewhenPlayed == "Monster": #只有随从打出的时候会计入打出的随从数
					self.Counters.numMonstersPlayedThisTurn[self.turn] += 1
				self.monsterPlayed = monster
				armedTrigs = self.armedTrigs("%sBeenPlayed"%typewhenPlayed)
				if self.GUI:
					self.GUI.showOffBoardTrig(monster, alsoDisplayCard=True, animate=False)
					self.GUI.hand2BoardAni(monster)
				target = monster.played(target, choice, mana, posinHand, comment)
				if self.monsterPlayed and self.monsterPlayed.onBoard:
					self.sendSignal("%sBeenSummoned"%typewhenPlayed, self.turn, self.monsterPlayed, target, mana, "")
					if typewhenPlayed=="Monster":
						self.Counters.numMonstersSummonedThisGame[self.monsterPlayed.camp] += 1
				self.Counters.numCardsPlayedThisTurn[self.turn] += 1
				#假设打出的随从被对面控制的话仍然会计为我方使用的随从。被对方变形之后仍记录打出的初始随从
				self.Counters.cardsPlayedThisTurn[self.turn]["Indices"].append(monsterIndex)
				self.Counters.cardsPlayedThisTurn[self.turn]["ManasPaid"].append(mana)
				self.Counters.cardsPlayedThisGame[self.turn].append(monsterIndex)
				if "~Corrupted~" in monster.index: self.Counters.corruptedCardsPlayed[self.turn].append(monsterIndex)
				if monster.name.endswith("Watch Post"): self.Counters.numWatchPostSummoned[self.turn] += 1
				#使用后步骤，触发镜像实体，狙击，顽石元素等“每当你使用一张xx牌”之后的扳机。
				if self.monsterPlayed and self.monsterPlayed.type != "Dormant":
					if self.monsterPlayed.creator: self.Counters.createdCardsPlayedThisGame[self.turn] += 1
					#The comment here is posinHand, which records the position a card is played from hand. -1 means the rightmost, 0 means the leftmost
					self.sendSignal("%sBeenPlayed"%typewhenPlayed, self.turn, self.monsterPlayed, target, mana, posinHand, choice, armedTrigs)
			#............完成阶段结束，开始处理死亡情况，此时可以处理胜负问题。
			self.gathertheDead(True)
			for card in self.Hand_Deck.hands[1] + self.Hand_Deck.hands[2]:
				card.effCanTrig()
				card.checkEvanescent()
			if not isinstance(tarIndex, list):
				self.moves.append(("playMonster", subIndex, subWhere, tarIndex, tarWhere, position, choice))
			else:
				self.moves.append(("playMonster", subIndex, subWhere, tuple(tarIndex), tuple(tarWhere), position, choice))
			print("Changing the game moves", self.moves)
			
	#召唤随从会成为夹杂在其他的玩家行为中，不视为一个完整的阶段。也不直接触发亡语结算等。
	#This method can also summon monsters for enemy.
	#SUMMONING MONSTERS ONLY CONScampERS ONBOARD MONSTERS. MONSTERS THAT HAVE ENTERED THE tempDeads DON'T COUNT AS MONSTERS.
	...
	def summonx2(self, subject, position, summoner):
		if not isinstance(subject, (list, np.ndarray)): #Summon a single monster
			if self.space(subject.camp) > 0:
				newSubjects = [subject, subject.selfCopy(subject.camp)]
				pos = [position, position]
				self.summon(newSubjects, pos, summoner)
		elif len(subject) == 1: #A list that has only 1 monster to summon
			if self.space(subject[0].camp) > 0:
				self.summonx2(subject[0], position[0], summoner) #Go back to doubling a single monster
		else:
			if self.space(subject[0].camp) > 0:
				newSubjects = []
				newPositions = []
				...
				if position[1] == "totheRightEnd":
					for sub in subject:
						...
						newSubjects.append(sub.selfCopy(sub.camp))
						newPositions.append(-1)
						newPositions.append(-1)
				elif position[1] == "leftandRight": #Can only be even: 2, 4, 6
					if num_orig == 2:
						newSubjects = [subject[0], subject[0].selfCopy(subject[0].camp),
										subject[1], subject[1].selfCopy(subject[1].camp)]
						newPositions = [pos+1, pos+1, pos, pos]
					else: #num_orig == 4 or 6
						newSubjects = [subject[0], subject[0].selfCopy(subject[0].camp),
										subject[1], subject[1].selfCopy(subject[1].camp),
										subject[2], subject[2].selfCopy(subject[2].camp)]
					...
					for i in range(min(4, num_orig)): #Deathrattle summoning is also handled here.
						...
						newSubjects.append(subject[i].selfCopy(subject[i].camp))
	...
	#returns the single monster summoned. Used for anchoring the position of the original monster summoned during doubling
	#fromHandDeck {created: 0, fromHand: 1, fromDeck: 2}
	def summonSingle(self, subject, position, summoner, fromHandDeck=0):
		camp = subject.camp
		if self.space(camp) > 0:
			subject.creator = summoner
			subject.seq = len(self.monsters[1]) + len(self.monsters[2]) + len(self.weapons[1]) + len(self.weapons[2])
			self.monsters[subject.camp].insert(position+100*(position<0), subject)  #If position is too large, the insert() simply puts it at the end.
			self.sortPos()
			self.sortSeq()
			if self.GUI:
				if fromHandDeck == 1: self.GUI.hand2BoardAni(subject)
				elif fromHandDeck == 2: self.GUI.deck2BoardAni(subject)
				else: self.GUI.monsterAppearAni(subject)
			subject.appears()
			self.Counters.numWatchPostSummoned[camp] += 1
			if subject.type=="Monster":
				self.sendSignal("MonsterSummoned", self.turn, subject, None, 0, "")
				self.sendSignal("MonsterBeenSummoned", self.turn, subject, None, 0, "")
				self.Counters.numMonstersSummonedThisGame[subject.camp] += 1
	...
	#return the first monster summoned, even if subject is list. Return None if no space left
	def summon(self, subject, position, summoner):
		summonercamp = summoner.camp
		if not isinstance(subject, (list, np.ndarray)): #Summon a single monster
			camp, timesofx2 = subject.camp, self.status[summonercamp]["Summon x2"]
			if summoner and summoner.type == "Power": timesofx2 = 0 #如果是英雄技能进行的召唤，则不会翻倍。
			if timesofx2 > 0:
				numCopies = 2 ** timesofx2 - 1
				copies = [subject.selfCopy(camp) for i in range(numCopies)]
				monsterSummoned = self.summonSingle(subject, position, summoner)
				if monsterSummoned: #只有最初的本体召唤成功的时候才会进行复制的随从的召唤
					...
					return monsterSummoned #只要第一次召唤出随从就视为召唤成功
				return None
			else: return self.summonSingle(subject, position, summoner)
		else: #Summoning multiple monsters in a row. But the list can be of length 1
			if len(subject) == 1: #用列表形式但是只召唤一个随从的时候，position一定是(self.pos, "totheRight")或者（-1, "totheRightEnd"）
				...
				if position[1] == "leftandRight":
					centralMonster, totheRight = self.monsters[subject[0].camp][position[0]], 1 #必须得到中间的随从的位置
					for i in range(len(subject)):
						if i == 0: pos = centralMonster.pos+1 #i == 0 召唤的第一个随从直接出现在传递进来的位置的右+1，没有任何问题。但是之后的召唤需要得知发起随从的位置或者之前召唤的随从的位置
						elif i == 1: pos = centralMonster.pos #这个召唤实际上是在列表中插入一个新的随从把中间随从向右挤
	...
	def summonfrom(self, i, camp, position, summoner, fromHand=True):
		if self.space(camp) > 0:
			if fromHand: subject = self.Hand_Deck.extractfromHand(i, camp, all=False, enemyCanSee=True)[0]
			else: subject = self.Hand_Deck.extractfromDeck(i, camp, all=False, enemyCanSee=True)[0]
			summonercamp = summoner.camp
			camp, timesofx2 = subject.camp, self.status[summonercamp]["Summon x2"]
			if summoner and summoner.type == "Power": timesofx2 = 0 #如果是英雄技能进行的召唤，则不会翻倍。
			if timesofx2 > 0:
	...
	def killMonster(self, subject, target):
		if isinstance(target, (list, tuple, np.ndarray)):
			...
			elif target.inHand: self.Hand_Deck.discard(target.camp, target)  # 如果随从在手牌中则将其丢弃

	def necromancy(self, subject, camp, number):
		if self.Counters.shadows[camp] >= number:
			self.Counters.shadows[camp] -= number
			self.sendSignal("Necromancy", camp, subject, None, number, "")
			return True
		return False

	def transform(self, target, newMonster, firstTime=True):
		camp = target.camp
		if target in self.monsters[camp]:
			pos = target.pos
			target.disappears(deathrattlesStayArmed=False, disappearResponse=False)
			self.removeMonsterorWeapon(target, animate=False)
			if self.monsterPlayed == target: self.monsterPlayed = newMonster
			#removeMonsterorWeapon invokes sortPos() and sortSeq()
			newMonster.seq = len(self.monsters[1]) + len(self.monsters[2]) + len(self.weapons[1]) + len(self.weapons[2])
			self.monsters[camp].insert(pos, newMonster)
			self.sortPos()
			if self.GUI:
				if newMonster.type == target.type:
					target.btn.changeCard(newMonster)
				else: #主要用于随从和休眠物之间的变形
					btn = target.btn
					boardZone = self.GUI.boardZones[camp]
					pos = target.btn.getPos()
					boardZone.addaMonster(newMonster, pos)
					boardZone.removeMonster(btn)
					self.GUI.ensureBtnMovedAway(btn)
			newMonster.appears(firstTime)
		elif target in self.Hand_Deck.hands[target.camp]:
			if self.monsterPlayed == target: self.monsterPlayed = newMonster
			self.Hand_Deck.replaceCardinHand(target, newMonster)
		
	#This method is always invoked after the monster.disappears() method.
	def removeMonsterorWeapon(self, target, animate=True):
		target.onBoard = False
		zone = self.weapons[target.camp] if target.type == "Weapon" else self.monsters[target.camp]
		try: zone.remove(target)
		except: pass
		self.sortSeq()
		if target.type != "Weapon": self.sortPos()
		if self.GUI and animate:
			self.GUI.removeMonsterorWeaponAni(target)

	def banishMonster(self, subject, target):
		if target:
			if isinstance(target, list):
				if self.GUI and subject: self.GUI.AOEAni(subject, target, ['○']*len(target), color="grey46")
				for obj in target:
					obj.disappears(deathrattlesStayArmed=False)
					self.removeMonsterorWeapon(obj)
			else:
				...
				if target.onBoard:
					...
					self.removeMonsterorWeapon(target)
				elif target.inHand: self.Hand_Deck.extractfromHand(target.camp, target) #如果随从在手牌中则将其丢弃


	def isVengeance(self, camp):
		return self.heroes[camp].health <= 10 or self.Counters.tempVengeance[camp]

	def isAvarice(self, camp):
		return self.Counters.numCardsDrawnThisTurn[camp] >= 2

	def isWrath(self, camp):
		return self.Counters.timesHeroTookDamage_inOwnTurn[camp] >= 7

	def isResonance(self, camp):
		return len(self.Hand_Deck.decks[camp]) % 2 == 0

	def isOverflow(self, camp):
		return self.Manas.manasUpper[camp] >= 7

	def combCards(self, camp):
		return self.Counters.numCardsPlayedThisTurn[camp]

	def getEvolutionPoint(self, camp):
		point = 0
		if self.heroes[camp].heroPower.name == "Evolve" and self.Counters.turns[camp] >= \
				self.Counters.numEvolutionTurn[camp]:
			point = self.Counters.numEvolutionPoint[camp]
		return point

	def restoreEvolvePoint(self, camp, number=1):
		if self.heroes[camp].heroPower.name == "Evolve":
			self.Counters.numEvolutionPoint[camp] = min(1 + camp, self.Counters.numEvolutionPoint[camp] + number)

	def earthRite(self, subject, camp, number=1):
		earths = self.earthsonBoard(camp)
		if len(earths) >= number:
			for i in range(number):
				self.killMonster(subject, earths[i])
				self.gathertheDead()
			return True
		return False

	def reanimate(self, camp, mana):
		if self.mode == 0:
			t = None
			if self.guides:
				t = self.cardPool[self.guides.pop(0)]
			else:
				indices = self.Counters.monstersDiedThisGame[camp]
				monsters = {}
				for index in indices:
					try:
						monsters[self.cardPool[index].mana].append(self.cardPool[index])
					except:
						monsters[self.cardPool[index].mana] = [self.cardPool[index]]
				for i in range(mana, -1, -1):
					if i in monsters:
						t = np.random.choice(monsters[i])
						break
				self.fixedGuides.append(t.index)
			if t:
				subject = t(self, camp)
				self.summon([subject], (-1, "totheRightEnd"), None)
				self.sendSignal("Reanimate", camp, subject, None, mana, "")
				return subject
		return None

	#The leftmost monster has position 0. Consider Dormant
	def sortPos(self):
		for i, obj in enumerate(self.monsters[1]): obj.pos = i
		for i, obj in enumerate(self.monsters[2]): obj.pos = i

	#Rearrange all livng monsters' sequences if change is true. Otherwise, just return the list of the sequences.
	#需要考虑Dormant的出场顺序
	def sortSeq(self):
		objs = self.weapons[1] + self.weapons[2] + self.monsters[1] + self.monsters[2]
		for i, obj in zip(np.asarray([obj.seq for obj in objs]).argsort().argsort(), objs): obj.seq = i

	def returnMonstertoHand(self, target, deathrattlesStayArmed=False, manaMod=None):
		camp = target.camp
		if target in self.monsters[camp]: #如果随从仍在随从列表中
			if self.Hand_Deck.handNotFull(camp):
				...
				self.Hand_Deck.addCardtoHand(target, camp)
			...
			Copy = type(target)(self, camp)
			self.Hand_Deck.addCardtoHand(Copy, camp)
			return Copy
		elif target.inHand: return target
		else: return None #The target is dead and removed already

	#targetDeckcamp decides the destination. initiatorcamp is for triggers, such as Trig_AugmentedElekk
	def returnMonstertoDeck(self, target, targetDeckcamp, initiatorcamp, deathrattlesStayArmed=False):
		if target in self.monsters[target.camp]:
			#如果onBoard仍为True，则其仍计为场上存活的随从，需调用disappears以注销各种扳机
			if target.onBoard: #随从存活状态下触发死亡扳机的区域移动效果时，不会注销其他扳机
				target.disappears(deathrattlesStayArmed)
			#如onBoard为False，则disappears已被调用过了。主要适用于触发死亡扳机中的区域移动效果
			self.removeMonsterorWeapon(target)
			target.reset(targetDeckcamp) #永恒祭司的亡语会备份一套enchantment，在调用该函数之后将初始化过的本体重新增益
	...
	def monsterSwitchSide(self, target, activity="Permanent"):
		#如果随从在手牌中，则该会在手牌中换边；如果随从在牌库中，则无事发生。
		if target.inHand and target in self.Hand_Deck.hands[target.camp]:
			card, mana, posinHand = self.Hand_Deck.extractfromHand(target, enemyCanSee=True)
			#addCardtoHand method will force the camp of the card to change to the target hand camp.
			#If the other side has full hand, then the card is extracted and thrown away.
			self.Hand_Deck.addCardtoHand(card, 3-card.camp)
		elif target.onBoard: #If the monster is on board.
			if self.space(3-target.camp) < 1:
				target.dead = True
			else:
				target.disappears(deathrattlesStayArmed=False) #随从控制权的变更会注销其死亡扳机，随从会在另一方重新注册其所有死亡扳机
				self.monsters[target.camp].remove(target)
				target.camp = 3 - target.camp
				self.monsters[target.camp].append(target)
				...
				if activity == "Borrow":
					if target.status["Borrowed"] < 1:
						...
						#因为回合结束扳机的性质，只有第一个同类扳机会触发，因为后面的扳机检测时会因为camp已经不同于当前回合的camp而不能继续触发
	...
	def armedTrigs(self, sig):
		trigs = []
		for camp in [self.mainPlayercamp, 3 - self.mainPlayercamp]:
			try: trigs += self.trigsBoard[camp][sig]
			except: pass
			try: trigs += self.trigsHand[camp][sig]
			except: pass
			try: trigs += self.trigsDeck[camp][sig]
			except: pass
		return trigs

	#New signal processing can be interpolated during the processing of old signal
	def sendSignal(self, signal, camp, subject, target, number, comment, choice=0, trigPool=None):
		hasResponder = False
		if trigPool is not None: #主要用于打出xx牌和随从死亡时/后扳机，它们有预检测机制。
			for trig in trigPool: #扳机只有仍被注册情况下才能触发，但是这个状态可以通过canTrigger来判断，而不必在所有扳机列表中再次检查。
				if trig.canTrig(signal, camp, subject, target, number, comment, choice): #扳机能触发通常需要扳机记录的实体还在场上等。
					hasResponder = True
					trig.trig(signal, camp, subject, target, number, comment, choice)
		else: #向所有注册的扳机请求触发。
			mainPlayercamp = self.mainPlayercamp #假设主副玩家不会在一次扳机结算之中发生变化。先触发主玩家的各个位置的扳机。
			#Trigger the trigs on main player's side, in the following order board-> hand -> deck.
			for triggercamp in [mainPlayercamp, 3-mainPlayercamp]:
				trigs = [] #TrigsBoard
				try: #这个信号有扳机在监听时，记录所有满足条件的扳机
					trigs = [trig for trig in self.trigsBoard[triggercamp][signal] if trig.canTrig(signal, camp, subject, target, number, comment, choice)]
					#某个随从死亡导致的队列中，作为场上扳机，救赎拥有最低优先级，其始终在最后结算
					if trigs: hasResponder = True
					#Redemption has been rotated out
					#if signal == "MonsterDies" and self.Secrets.sameSecretExists(Redemption, 3-self.turn):
					#	for i in range(len(trigs)):
					#		if type(trigs[i]) == Trig_Redemption:
					#			trigs.append(trigs.pop(i)) #把救赎的扳机移到最后
					#			break
					for trig in trigs: trig.trig(signal, camp, subject, target, number, comment, choice)
				except: pass
				try: #TrigsHand
					trigs = [trig for trig in self.trigsHand[triggercamp][signal] if trig.canTrig(signal, camp, subject, target, number, comment, choice)]
					if trigs: hasResponder = True
					for trig in trigs: trig.trig(signal, camp, subject, target, number, comment, choice)
				except: pass
				try: #TrigsDeck
					trigs = [trig for trig in self.trigsDeck[triggercamp][signal] if trig.canTrig(signal, camp, subject, target, number, comment, choice)]
					if trigs: hasResponder = True
					invocation = []
					for trig in trigs:
	...
	#The weapon will also join the deathList and compare its own sequence against other monsters.
	def gathertheDead(self, decideWinner=False):
		...
		for camp in range(1, 3):
			...
			while self.weapons[camp]:
				if self.weapons[camp][0].durability < 1 or self.weapons[camp][0].dead:
					self.weapons[camp][0].destroyed() #武器的被摧毁函数，负责其onBoard, dead和英雄风怒，攻击力和场上扳机的移除等。
					weapon = self.weapons[camp].pop(0)
					self.Counters.weaponsDestroyedThisGame[weapon.camp].append(weapon.index)
					self.tempDeads[0].append(weapon)
					self.tempDeads[1].append(weapon.attack)
				else: #If the weapon is the latest weapon to equip
					break
			for monster in self.monstersonBoard(camp) + self.amuletsonBoard(camp):
				if monster.dead and monster.marks["Can't Break"] > 0:
					monster.dead = False
				if monster.type == "Monster":
					if monster.health < 1 or monster.dead:
						if monster.marks["Disappear When Die"] > 0:
							self.banishMonster(None, monster)
							continue
						monster.dead = True
						self.tempDeads[0].append(monster)
						self.tempDeads[1].append(monster.attack)
						monster.disappears(deathrattlesStayArmed=True) #随从死亡时不会注销其死亡扳机，这些扳机会在触发之后自行注销
						self.Counters.monstersDiedThisTurn[monster.camp].append(monster.index)
						self.Counters.monstersDiedThisGame[monster.camp].append(monster.index)
						if "Artifact" in monster.race:
							if monster.index in self.Counters.artifactsDiedThisGame[monster.camp]:
								self.Counters.artifactsDiedThisGame[monster.camp][monster.index] += 1
							else:
								self.Counters.artifactsDiedThisGame[monster.camp][monster.index] = 1
						self.Counters.shadows[monster.camp] += 1
				elif monster.dead: #The obj is Amulet and it's been marked dead
					monster.dead = True
					self.tempDeads[0].append(monster)
					self.tempDeads[1].append(0)
					monster.disappears(deathrattlesStayArmed=True) #随从死亡时不会注销其死亡扳机，这些扳机会在触发之后自行注销
					self.Counters.amuletsDestroyedThisTurn[monster.camp].append(monster.index)
					self.Counters.amuletsDestroyedThisGame[monster.camp].append(monster.index)
					self.Counters.shadows[monster.camp] += 1
			#无论是不在此时结算胜负，都要在英雄的生命值降为0时将其标记为dead
			if self.heroes[camp].health < 1: self.heroes[camp].dead = True

		if self.tempDeads[0]: #self.tempDeads != [[], []]
			#Rearrange the dead monsters according to their sequences.
			self.tempDeads[0], order = self.orderofDead(self.tempDeads[0])
			temp = self.tempDeads[1]
			self.tempDeads[1] = []
			for i in range(len(order)):
				self.tempDeads[1].append(temp[order[i]])
			if self.GUI: self.GUI.monstersDieAni(self.tempDeads[0])
	...
	def deathHandle(self, decideWinner=False):
		while True:
			rebornMonsters = []
			if not self.deads[0]: break #If no monsters are dead, then stop the loop
			armedTrigs_WhenDies = self.armedTrigs("MonsterDies") + self.armedTrigs("WeaponDestroyed") + self.armedTrigs("AmuletDestroys")
			armedTrigs_AfterDied = self.armedTrigs("MonsterDied") + self.armedTrigs("AmuletDestroyed")
			while self.deads != [[], []]:
				...
				#weapons with regard to deathrattle triggering is handled the same way as monsters.
				...
				print("Removing dead monster/weapon before deathresolution", objtoDie, objtoDie.btn)
				if objtoDie.type == "Monster" and objtoDie.keyWords["Reborn"] > 0: rebornMonsters.append(objtoDie)
				objtoDie.deathResolution(attackwhenDies, armedTrigs_WhenDies, armedTrigs_AfterDied)
				print("Removing dead monster/weapon", objtoDie, objtoDie.btn)
				self.removeMonsterorWeapon(objtoDie) #结算完一个随从的亡语之后将其移除。
				objtoDie.reset(objtoDie.camp)
				objtoDie.dead = True
				self.deads[0].pop(0)
				self.deads[1].pop(0)
			#当一轮死亡结算结束之后，召唤这次死亡结算中死亡的复生随从
			for rebornMonster in rebornMonsters:
				monstertoSummon = type(rebornMonster)(self, rebornMonster.camp)
				monstertoSummon.keyWords["Reborn"], monstertoSummon.health = 0, 1 #不需要特殊的身材处理，激怒等直接在随从的appears()函数中处理。
				self.summon(monstertoSummon, rebornMonster.pos, rebornMonster)
		...
		self.resolvingDeath = False
		#The reborn effect take place after the deathrattles of monsters have been triggered.
	...
	"""
	At the start of turn, the AOE destroy/AOE damage/damage effect won't kill monsters make them leave the board.
	As long as the monster is still on board, it can still trigger its turn start/end effects.
	Special things are Sap/Defile, which will force the monster to leave board early.
	#The Defile will cause the game to preemptively start death resolution.
	Archmage casting spell will be able to target monsters with health <= 0, since they are not regarded as dead yet.
	The deaths of monsters will be handled at the end of triggering, which is then followed by drawing card.
	"""
	def switchTurn(self):
		if self.GUI: self.GUI.switchTurnAni()
		for monster in self.monsters[self.turn] + self.monsters[3-self.turn]: #Include the Dormants.
			monster.turnEnds(self.turn) #Handle monsters' attTimes and attChances
		for card in self.Hand_Deck.hands[self.turn]	+ self.Hand_Deck.hands[3-self.turn]:
			if card.type == "Monster": #Monsters in hands will clear their temp buffDebuff
				card.turnEnds(self.turn) #Monsters in hands can't defrost
		...
		for obj in self.turnStartTrigger[:]: #This is for temp effects.
			if not hasattr(obj, "camp") or obj.camp == self.turn: obj.turnStartTrigger()
		...
		for monster in self.monsters[1] + self.monsters[2]: #Include the Dormants.
			monster.turnStarts(self.turn) #Handle monsters' attTimes and attChances
		for card in self.Hand_Deck.hands[1]	+ self.Hand_Deck.hands[2]:
	...
	def battle(self, subject, target, verifySelectable=True, useAttChance=True, resolveDeath=True, resetRedirTrig=True):
		...
		#如果有攻击之后的发现效果需要结算，则在此结算之后。
			...
			if verifySelectable:
				subIndex, subWhere = subject.pos, subject.type+str(subject.camp)
				tarIndex, tarWhere = target.pos, target.type+str(target.camp)
			...
			if ((subject.type != "Monster" and subject.type != "Hero") or not subject.onBoard or subject.health < 1 or subject.dead) \
				or ((target.type != "Monster" and target.type != "Hero") or not target.onBoard or target.health < 1 or target.dead) \
			...
			if battleContinues:
				...
				if subject == self.heroes[1] or subject == self.heroes[2]:
					self.Counters.heroAttackTimesThisTurn[subject.camp] += 1
	...
	def playSpell(self, spell, target, choice=0, comment=""):
		...
		if self.Manas.affordable(spell) and spell.available() and spell.selectionLegit(target, choice):
			...
			subIndex, subWhere = self.Hand_Deck.hands[spell.camp].index(spell), "Hand%d"%spell.camp
			if target:
				if isinstance(target, list):
					tarIndex, tarWhere = [], []
					for obj in target:
	...
	def availableWeapon(self, camp):
		return next((weapon for weapon in self.weapons[camp] if weapon.durability > 0 and weapon.onBoard), None)
	...
	def playWeapon(self, weapon, target, choice=0):
		...
		if self.Manas.affordable(weapon):
			...
			subIndex, subWhere = self.Hand_Deck.hands[weapon.camp].index(weapon), "Hand%d"%weapon.camp
			if target:
				tarIndex, tarWhere = target.pos, target.type+str(target.camp)
			else: tarIndex, tarWhere = 0, ''
			#卡牌从手中离开，支付费用，费用状态移除，但是目前没有根据武器费用支付而产生响应的效果。
			...
			self.Counters.numCardsPlayedThisTurn[camp] += 1
			self.Counters.cardsPlayedThisTurn[camp]["Indices"].append(weaponIndex)
			self.Counters.cardsPlayedThisTurn[camp]["ManasPaid"].append(mana)
			self.Counters.cardsPlayedThisGame[camp].append(weaponIndex)
			...
			if weapon.creator: self.Counters.createdCardsPlayedThisGame[camp] += 1
	...
	def equipWeapon(self, weapon):
		camp = weapon.camp
		if self.weapons[camp] != []: #There are currently weapons before it.
			for obj in self.weapons[camp]:
		...
		self.weapons[camp].append(weapon)
		weapon.onBoard = True
	...
	def playHero(self, heroCard, choice=0):
		...
		if self.Manas.affordable(heroCard):
			...
			subIndex, subWhere = self.Hand_Deck.hands[heroCard.camp].index(heroCard), "Hand%d"%heroCard.camp
			#支付费用，以及费用状态移除
			...
			self.Counters.numCardsPlayedThisTurn[camp] += 1
			self.Counters.cardsPlayedThisTurn[camp]["Indices"].append(heroCardIndex)
			self.Counters.cardsPlayedThisTurn[camp]["ManasPaid"].append(mana)
			self.Counters.cardsPlayedThisGame[camp].append(heroCardIndex)
		#完成阶段
	...
	def copyGame(self, num=1):
		...
		for Copy in copies:
			Copy.copiedObjs = {}
			Copy.mainPlayercamp, Copy.GUI = self.mainPlayercamp, self.GUI
			Copy.cardPool, Copy.ClassCards, Copy.NeutralCards = self.cardPool, self.ClassCards, self.NeutralCards
			Copy.MonstersofCost, Copy.RNGPools = self.MonstersofCost, self.RNGPools
			#t1 = datetime.now()
			...
			Copy.monsters = {1: [monster.createCopy(Copy) for monster in self.monsters[1]],
							2: [monster.createCopy(Copy) for monster in self.monsters[2]]}
			#t2 = datetime.now()
			#print("Time to copy characters onBoard", datetime.timestamp(t2)-datetime.timestamp(t1))
			#t1 = datetime.now()
			Copy.trigAuras = {camp: [aura.createCopy(Copy) for aura in self.trigAuras[camp]] for camp in range(1, 3)}
			...
			Copy.monsterPlayed, Copy.gameEnds, Copy.turn, Copy.numTurn = None, self.gameEnds, self.turn, self.numTurn
			...
			for trigs1, trigs2 in zip([Copy.trigsBoard, Copy.trigsHand, Copy.trigsDeck], [self.trigsBoard, self.trigsHand, self.trigsDeck]):
				for camp in range(1, 3):
					for sig in trigs2[camp].keys(): trigs1[camp][sig] = [trig.createCopy(Copy) for trig in trigs2[camp][sig]]
	...
	def find(self, i, where):
		return {"Monster1": self.monsters[1],
				"Monster2": self.monsters[2],
				"Amulet1": self.monsters[1],
				"Amulet2": self.monsters[2],
				"Hero1": self.heroes, #For heroes, their position is kept the same as their camp
				"Hero2": self.heroes,
				"Power": self.powers,
	...
	def decodePlay(self, move):
		if move[0] == "EndTurn": self.switchTurn()
			...
			"playMonster": lambda: self.playMonster(sub, tar, move[5], move[6]),
